...
const ModalPopup: FunctionComponent<PopupProps> = ({
  children,
  overlayColor,
  placement = "Centered",
  onOutsideClick,
  ...
  const onOverlayClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (
        onOutsideClick &&
        (e.target as HTMLElement).classList.contains("modalPopupOverlay")
      ) {
        onOutsideClick();
      }
      e.stopPropagation();
  ...
  return (
    <Portal>
      <div
        className="modalPopupOverlay fixed inset-0 flex flex-col"
        style={popupStyle}
